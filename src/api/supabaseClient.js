...
function createEntity(entityName) {
  ...
  return {
    ...
    async create(payload) {
      ...
      if (!userId) {
        throw new Error(
          'Vous devez être connecté pour créer des données.'
        );
      }
...
export const base44 = {
  ...
  entities: Object.fromEntries(
    Object.keys(entityTableNames).map((name) => [
      name,
      createEntity(name),
    ]),
  ),
  ...
  integrations: {
    Core: {
      ...
      // ==========================================
      // UPLOAD DE FICHIER → SUPABASE STORAGE
      // ==========================================
      ...
      async InvokeLLM() {
        throw new Error(
          'InvokeLLM doit être connecté à votre fournisseur IA.'
        );
      },
    },
  },
